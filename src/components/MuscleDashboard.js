...
function PRBadge({ value, color }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: '13px', fontWeight: '600', color: '#fff', opacity: 0.85, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Personal Record</span>
      <span style={{ fontSize: '48px', fontWeight: '800', color: '#fff', lineHeight: 1.1 }}>{value}</span>
      <span style={{ fontSize: '16px', fontWeight: '500', color: '#fff', opacity: 0.85 }}>kg</span>
    </div>
  );
}
...
function ChartCard({ title, url }) {
  return (
    <div style={cardStyle}>
      <h3 style={{ margin: '0 0 16px', fontSize: '15px', fontWeight: '700', color: APP_COLORS.text }}>{title}</h3>
      {url ? (
        <iframe
          src={url}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: '13px', fontWeight: '600', color: '#fff', opacity: 0.8, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{exercise}</div>
          <div style={{ fontSize: isMobile ? '22px' : '32px', fontWeight: '800', color: '#fff' }}>{label}</div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', flex: 1 }}>
            <div style={{ ...cardStyle, marginBottom: 0, display: 'flex', gap: '24px', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: '28px', fontWeight: '700', color: APP_COLORS.text }}>{startWeight} <span style={{ fontSize: '14px', color: APP_COLORS.textLight }}>kg</span></div>
              </div>
              ...
              <div>
                ...
                <div style={{ fontSize: '28px', fontWeight: '700', color: color }}>+{totalGain.toFixed(2)} <span style={{ fontSize: '14px', color: APP_COLORS.textLight }}>kg</span></div>
              </div>
              <div>
